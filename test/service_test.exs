...
defmodule ServiceTest do
	...
	test :invalid_password_request do
		...
		assert { :error, :invalid_request } == MockService.grant_access(request)
		...
		assert { :error, :invalid_request } = MockService.grant_access(Keyword.put request, :username, "user")
		assert { :error, :invalid_request } = MockService.grant_access(Keyword.put request, :password, "password")
	end

	test :password_grant do
		failing_request = [ grant_type: :password, client_id: "client", client_secret: "shh!" ]
		assert { :error, :invalid_client } == MockService.grant_access(failing_request)
	...
	test :client_credentials_grant do
		...
		assert { :error, :invalid_request } = MockService.grant_access(failing_request)
	...
	test :unsupported_grant_type do
		...
		assert { :error, :unsupported_grant_type } = MockService.grant_access(request)
	end
end
